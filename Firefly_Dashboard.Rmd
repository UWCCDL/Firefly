...
prior_knowledge_alldata$BLC <- NULL  # Remove OLD BLC data
...
ggplot(prior_agg, aes(x=no, y=yes)) +
  ...
  annotate(geom="text", x=0.25, y =0.35, 
           label=paste("r =", round(cor(prior_agg$no, prior_agg$yes),
                                    2))) +
...
recovered_blc <- inner_join(recovered_blc, prior_knowledge_alldata)
